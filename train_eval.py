...
class StepRunner:
    ...
    def step(self, features, labels):
        ...
        step_metrics = {
            ...
            name: metric_fn(
                preds,
                labels).item() for name,
            metric_fn in self.metrics_dict.items()}
        if self.stage == "test":
    ...
    def __call__(self, dataloader):
        ...
        for i, batch in loop:
            ...
            if i != len(dataloader) - 1:
                ...
                epoch_metrics = {
                    self.stage + "_" + name: metric_fn.compute().item() for name,
                    metric_fn in self.step_runner.metrics_dict.items()}
                epoch_log = dict(
